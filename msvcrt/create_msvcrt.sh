# This script must be called with mingw in the PATH before cygwin, otherwise
# the result will be garbage
MINGW_PATH=/cygdrive/c/Mingw32/bin
PATH=$MINGW_PATH:$PATH
if test ! -f "${MINGW_PATH}/gcc.exe"; then
	echo "gcc.exe not found in the mingw path ${MINGW_PATH} !"
	exit -1;
fi;
